...
export function createCelestialVisualsController(deps) {
  ...
  const tempObserverUpAxis = new THREE.Vector3();
  ...
  function updateRenderState() {
    ...
    globeStage?.scale.setScalar(renderState.visualScale);
  }
  ...
  function getObserverSkyAxes(observerPosition, observerGeo = getObserverGeo()) {
    ...
    tempObserverUpAxis.set(basis.up.x, basis.up.y, basis.up.z).normalize();
  }
  ...
  function positionBodyInObserverSky(body, horizontal, observerGeo) {
    ...
    getObserverSkyAxes(walkerState.position, observerGeo);
    tempObserverSkyOrigin
      .copy(walkerState.position)
      .addScaledVector(tempObserverUpAxis, WALKER_EYE_OFFSET)
      .multiplyScalar(renderState.visualScale);
    tempObserverSkyPoint.copy(tempObserverSkyOrigin)
      ...
      .addScaledVector(tempObserverUpAxis, Math.sin(horizontal.altitudeRadians) * skyDistance);
  ...
  function getHorizontalFromWorldPosition(sourceWorldPosition, observerGeo) {
    ...
    const upComponent = tempObserverRelative.dot(tempObserverUpAxis);
  ...
  function configurePreparationCamera(targetCamera) {
    ...
    tempPreparationLookTarget.copy(targetCamera.position)
      .addScaledVector(tempObserverNorthAxis, Math.cos(walkerState.heading) * horizontalDistance)
      .addScaledVector(tempObserverEastAxis, Math.sin(walkerState.heading) * horizontalDistance)
      .addScaledVector(tempObserverUpAxis, Math.sin(walkerState.pitch) * constants.WALKER_LOOK_DISTANCE);
    targetCamera.up.copy(tempObserverUpAxis);
